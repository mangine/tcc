...
public:
	...
	/////////////////////////////////////////////////////////////////////////////////////
	MFRC522(int chipSelectPin, int resetPowerDownPin, mraa::Spi * bus);
	void setSPIConfig();
	...
	std::string GetStatusCodeName(byte code);
	byte PICC_GetType(byte sak);
	// old function used too much memory, now name moved to flash; if you need char, copy from flash to memory
	//const char *PICC_GetTypeName(byte type);
	std::string PICC_GetTypeName(byte type);
	void PICC_DumpToSerial(Uid *uid, mraa::Uart * serial);
	void PICC_DumpMifareClassicToSerial(Uid *uid, byte piccType, MIFARE_Key *key,mraa::Uart *Serial);
	void PICC_DumpMifareClassicSectorToSerial(Uid *uid, MIFARE_Key *key, byte sector,mraa::Uart *Serial);
	void PICC_DumpMifareUltralightToSerial(mraa::Uart *Serial);
...
private:
	mraa::Gpio * _chipSelectPin;
	mraa::Gpio * _resetPowerDownPin;
	//byte _chipSelectPin;		// Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
	//byte _resetPowerDownPin;	// Arduino pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low)
	mraa::Spi * _bus;			// bus to be used
